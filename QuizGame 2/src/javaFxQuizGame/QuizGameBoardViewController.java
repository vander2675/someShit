...
public class QuizGameBoardViewController implements Initializable,
		IOverlayable, IObserver<GameState> {
	...
	public void initialize(URL location, ResourceBundle arg1) {
		...
		// add start images
		for (int i = 0; i < 4; i++) {
			ImageView startImgView = addImageToNode(
					getStartField(i),
					new Image(getClass().getResourceAsStream(
							"resources/Assets/arrow.png")), false);
	...
	private void setupDebugMode() {
		...
		MenuItem drawWS = new MenuItem("WS zu speziellem Feld ziehen");
		drawWS.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent e) {
				GameModel model = (GameModel) gameModel;
				model.makeDrawOccupied();
			}
		});
		debugMenu.getItems().add(drawWS);
	}
	...
	private void test() {
		...
		addImageToNode(currentPlayerCircle, new Image(getClass()
				.getResourceAsStream("resources/Assets/Fab.png")), true);
	}
	...
	private ImageView addWissensstreiterToField(Circle field, int playerNo) {
		...
		switch (playerNo) {
		case 0:
			wiImage = new Image(getClass().getResourceAsStream(
					"resources/Assets/Wissensstreiter/white.png"));
			break;

		case 1:
			wiImage = new Image(getClass().getResourceAsStream(
					"resources/Assets/Wissensstreiter/red.png"));
			break;

		case 2:
			wiImage = new Image(getClass().getResourceAsStream(
					"resources/Assets/Wissensstreiter/purple.png"));
			break;

		default:
			wiImage = new Image(getClass().getResourceAsStream(
					"resources/Assets/Wissensstreiter/green.png"));
			break;
		}
	...
	private void removeWissensstreiterz() {
		for (ImageView wissensstreiter : wissensstreiterz) {
			((Pane) wissensstreiter.getParent()).getChildren().remove(
					wissensstreiter);
		}
	...
	private ImageView addImageToNode(Node node, Image img,
			boolean fitSizeToParent) {
	...
	private void removeHighlightFromAllNodes() {
		if (highlightedNodes != null) {
			ArrayList<Node> highlightedNodesCopy = new ArrayList<>(
					highlightedNodes);
	...
	private void addGlowEffectToNode(Node node, Color color) {
		double depth = 70; // Setting the uniform variable for the glow width
							// and height
		...
		timeline.getKeyFrames().addAll(
				new KeyFrame(Duration.ZERO, // set start position at 0
						new KeyValue(borderGlow.widthProperty(), depth / 2.0,
								Interpolator.EASE_OUT), new KeyValue(
								borderGlow.heightProperty(), depth / 2.0,
								Interpolator.EASE_OUT)),
				new KeyFrame(new Duration(750), // set end position at 40s
						new KeyValue(borderGlow.widthProperty(), depth,
								Interpolator.EASE_OUT),
						new KeyValue(borderGlow.heightProperty(), depth,
								Interpolator.EASE_OUT)));

		timeline.play();

		node.setEffect(borderGlow); // Apply the borderGlow effect to the JavaFX
									// node
	}

	private void emphasizeGlowEffectFromNode(Node node, Color color,
	...
	private void initReferenceCollections() {
		...
		Circle[] fieldArray = { field0, field1, field2, field3, field4, field5,
				...
				field27, field28, field29, field30, field31, field32, field33,
				field34, field35, field36, field37, field38, field39, field40,
				field41, field42, field43, field44, field45, field46, field47 };
		this.fields = Arrays.asList(fieldArray);

		// categories
		Rectangle[] categoryArray = { category1Rect, category2Rect,
				category3Rect, category4Rect };
		this.categories = Arrays.asList(categoryArray);

		// turn-order-HUD
		Circle[] turnOrderCircleArray = { currentPlayerCircle, p2Circle,
				p3Circle, p4Circle };
		this.turnOrderCircles = Arrays.asList(turnOrderCircleArray);
		Label[] turnOrderLabelArray = { currentPlayerLabel, p2Label, p3Label,
				p4Label };
	...
	private void addPlayerElementsForPlayer1() {
		...
		List<Circle> cat1Fields = Arrays
				.asList(new Circle[] { waPlayer1Cat1Field0,
						waPlayer1Cat1Field1, waPlayer1Cat1Field2 });
		List<Circle> cat2Fields = Arrays
				.asList(new Circle[] { waPlayer1Cat2Field0,
						waPlayer1Cat2Field1, waPlayer1Cat2Field2 });
		List<Circle> cat3Fields = Arrays
				.asList(new Circle[] { waPlayer1Cat3Field0,
						waPlayer1Cat3Field1, waPlayer1Cat3Field2 });
		List<Circle> cat4Fields = Arrays
				.asList(new Circle[] { waPlayer1Cat4Field0,
						waPlayer1Cat4Field1, waPlayer1Cat4Field2 });
		...
		// Base Fields
		List<Circle> baseFields = Arrays.asList(new Circle[] { base0Player1,
				base1Player1, base2Player1 });
		playerMap.put(BASE_FIELDS_KEY, baseFields);

		// add playerMap to playerElements-dict, that collects player-elements
		// from ALL players.
	...
	private void addPlayerElementsForPlayer2() {
		...
		// Wissenstandsanzeiger
		List<Circle> cat1Fields = Arrays
				.asList(new Circle[] { waPlayer2Cat1Field0,
						waPlayer2Cat1Field1, waPlayer2Cat1Field2 });
		List<Circle> cat2Fields = Arrays
				.asList(new Circle[] { waPlayer2Cat2Field0,
						waPlayer2Cat2Field1, waPlayer2Cat2Field2 });
		List<Circle> cat3Fields = Arrays
				.asList(new Circle[] { waPlayer2Cat3Field0,
						waPlayer2Cat3Field1, waPlayer2Cat3Field2 });
		List<Circle> cat4Fields = Arrays
				.asList(new Circle[] { waPlayer2Cat4Field0,
						waPlayer2Cat4Field1, waPlayer2Cat4Field2 });
		// bundle into one array, which will later reside under the 'WA'-key
		List<List<Circle>> waFields = new ArrayList<List<Circle>>();
		waFields.add(cat1Fields);
		waFields.add(cat2Fields);
		...
		// Base Fields
		List<Circle> baseFields = Arrays.asList(new Circle[] { base0Player2,
				base1Player2, base2Player2 });
		playerMap.put(BASE_FIELDS_KEY, baseFields);
	...
	private void addPlayerElementsForPlayer3() {
		...
		// Wissenstandsanzeiger
		List<Circle> cat1Fields = Arrays
				.asList(new Circle[] { waPlayer3Cat1Field0,
						waPlayer3Cat1Field1, waPlayer3Cat1Field2 });
		List<Circle> cat2Fields = Arrays
				.asList(new Circle[] { waPlayer3Cat2Field0,
						waPlayer3Cat2Field1, waPlayer3Cat2Field2 });
		List<Circle> cat3Fields = Arrays
				.asList(new Circle[] { waPlayer3Cat3Field0,
						waPlayer3Cat3Field1, waPlayer3Cat3Field2 });
		List<Circle> cat4Fields = Arrays
				.asList(new Circle[] { waPlayer3Cat4Field0,
						waPlayer3Cat4Field1, waPlayer3Cat4Field2 });
		// bundle into one array, which will later reside under the 'WA'-key
		List<List<Circle>> waFields = new ArrayList<List<Circle>>();
		waFields.add(cat1Fields);
		waFields.add(cat2Fields);
		...
		// Base Fields
		List<Circle> baseFields = Arrays.asList(new Circle[] { base0Player3,
				base1Player3, base2Player3 });
		playerMap.put(BASE_FIELDS_KEY, baseFields);
	...
	private void addPlayerElementsForPlayer4() {
		...
		// Wissenstandsanzeiger
		List<Circle> cat1Fields = Arrays
				.asList(new Circle[] { waPlayer4Cat1Field0,
						waPlayer4Cat1Field1, waPlayer4Cat1Field2 });
		List<Circle> cat2Fields = Arrays
				.asList(new Circle[] { waPlayer4Cat2Field0,
						waPlayer4Cat2Field1, waPlayer4Cat2Field2 });
		List<Circle> cat3Fields = Arrays
				.asList(new Circle[] { waPlayer4Cat3Field0,
						waPlayer4Cat3Field1, waPlayer4Cat3Field2 });
		List<Circle> cat4Fields = Arrays
				.asList(new Circle[] { waPlayer4Cat4Field0,
						waPlayer4Cat4Field1, waPlayer4Cat4Field2 });
		// bundle into one array, which will later reside under the 'WA'-key
		List<List<Circle>> waFields = new ArrayList<List<Circle>>();
		waFields.add(cat1Fields);
		waFields.add(cat2Fields);
		...
		// Base Fields
		List<Circle> baseFields = Arrays.asList(new Circle[] { base0Player4,
				base1Player4, base2Player4 });
		playerMap.put(BASE_FIELDS_KEY, baseFields);
	...
	private void initClickHandlers() {
		...
		for (Map.Entry<String, Map<String, Object>> entry : playerElements
				.entrySet()) {
			...
			for (int i = 0; i < categories.size(); i++) {
				...
				for (Circle waField : waFields) {
					waField.setOnMouseClicked(new EventHandler<MouseEvent>() {
						@Override
						public void handle(MouseEvent t) {
							handleClickOnWAField(waField, playerNo,
									getWAFields(playerNo).indexOf(waFields));
						}
					});
				}
			}

			// for the (main) interaction button (throw dice | next turn | and
			// so on...)
	...
	private void handleClickOnField(Circle field) {
		...
		IWissensstreiter ws = IAPIFactory.factory.getSpielbrett()
				.getFieldAtIndex(indexOfField).getWissensstreiter().get(0);
	...
	private void handleClickOnCategoryField(Rectangle categoryField) {
		...
		emphasizeGlowEffectFromNode(categoryField, Color.web("#00FC00"),
				Color.RED);
	...
	private void handleClickOnBaseField(Circle baseField, int playerNo) {
		...
		System.out.println("baseField[" + indexOfField + "] of Player["
				+ playerNo + "] clicked!");
		int numberOfWSInHomeBase = IAPIFactory.factory.getPlayers()
				.getCountOfWissensstreiterInHomeBase(
						gameModel.getCurrentPlayer());
		if (indexOfField <= numberOfWSInHomeBase) {
			for (IWissensstreiter ws : gameModel.getCurrentPlayer()
					.getAllPlayerWissensstreiter().getWissenstreiter()) {
				if (IAPIFactory.factory.getSpielbrett()
						.getFieldOfWissensstreiter(ws) == null) {
					gameModel.chooseWS(ws);
				}
			}
		}
	...
	private void handleClickOnWAField(Circle waField, int playerNo,
			int categoryNo) {
		int indexOfField = this.getWAFields(playerNo, categoryNo).indexOf(
				waField);
		System.out.println("waField[" + indexOfField + "] of Player["
				+ playerNo + "] for Category[" + categoryNo + "] clicked!");
	...
	private void handleClickOnMainButton() {
		...
		case PROMPT_FOR_ACCEPT_CATEGORY:
			...
			ICategory selectedCategory = IAPIFactory.factory.getCategories()
					.getCategories().get(selectedCategoryNo);
	...
	private void setMainButtonToPromptForAcceptCategoryDecision(int categoryNo) {
		mainButton.setText("Kategorie " + Integer.toString(categoryNo)
				+ " bestätigen");
		mainButton.setGraphic(getAcceptPromptImageView());
		mainButtonState = MainButtonState.PROMPT_FOR_ACCEPT_CATEGORY;
	}

	private void updateTurnOrder(List<String> playerNames,
			List<Paint> playerColors, List<Image> playerImages) {
		for (int i = 0; i < this.playerCount; i++) {
			...
			if (playerImages != null && i < playerImages.size()) {
				ImageView imgView = this
						.getImageViewForNode(this.turnOrderCircles.get(i));

				if (imgView != null) {
					imgView.setImage(playerImages.get(i));
					this.fitImageViewToNodeSize(imgView,
							this.turnOrderCircles.get(i));
				} else {
					addImageToNode(this.turnOrderCircles.get(i),
							playerImages.get(i), true);
				}
			}
		}
	}
	...
	private void updateGameBoard() {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if (field.getWissensstreiter().size() > 0) {
				int playerNo = field.getWissensstreiter().get(0).getOwner()
						.getPlayerNumber();
		...
		for (IPlayer player : IAPIFactory.factory.getPlayers().getPlayers()) {
			...
			for (IWissensstreiter ws : player.getAllPlayerWissensstreiter()
					.getWissenstreiter()) {
				if (gameBoard.getFieldOfWissensstreiter(ws) == null) {
					addWissensstreiterToField(
							this.getBaseFields(player.getPlayerNumber()).get(
									basefieldIndex), player.getPlayerNumber());
					basefieldIndex++;
				}
			}
		}
		...
		for (IPlayer player : IAPIFactory.factory.getPlayers().getPlayers()) {
			for (int i = 0; i < IAPIFactory.factory.getCategories().count(); i++) {
				List<Circle> wa = getWAFields(player.getPlayerNumber(), i);
				int currentIndex = player.getWissenstandsanzeiger().getIndexForCategroyByCateNo(i);
				for (int j = 0; j < wa.size(); j++) {
					if(j < currentIndex) {
						wa.get(j).setFill(player.getPlayerColor());
					} else {
						wa.get(j).setFill(Color.TRANSPARENT);
					}
				}
			}
		}
	...
	private List<List<Circle>> getWAFields(int playerNo) {
		@SuppressWarnings("unchecked")
		List<List<Circle>> waCategories = (List<List<Circle>>) getPlayerMap(
				playerNo).get(WA_KEY);
		return waCategories;
	}
	...
	private ImageView getThrowDicePromptImageView() {
		Image img = new Image(getClass().getResourceAsStream(
				"resources/Assets/throwDicePrompt.png"));
	...
	private void overlayWithNodeFromFXML(String resourceName) {
		...
		try {
			...
			if (fxmlLoader.getController() != null) {
				viewControllerForOverlayViews.put(rootFromFXML,
		...
		// optionally further configurations following (if class is of type
	...
	private void willPresentViewController(Object viewController) {
		...
		if (viewController instanceof QuizGameQuestionViewController) {
			...
			ICategory selectedCategory = gameModel.getChoosenCategory();
			IQuestion selectedQuestion = gameModel.getQuestion();
			((QuizGameQuestionViewController) viewController)
					.setupWithParameters(
	...
	public void overlayWithNode(Node node, boolean yieldUserInteraction) {
		...
		if (yieldUserInteraction) {
			for (Node prevNode : rootPane.getChildren()) {
				// if a node is the borderPane, we want to ensure, that its
				// menuBar stays enabled
	...
	public void dismissOverlay(Node node) {
		...
		Node children = rootPane.getChildren().get(
	...
	public void update(GameState info) {
		...
		case NEW_TURN:
			List<IPlayer> playerOrder = IAPIFactory.factory
					.getPlayers()
					.getPlayerOrderByCurrentPlayer(gameModel.getCurrentPlayer());
		...
		case DICED_DRAWABLE:
			...
			ISpielbrett gameBoard = IAPIFactory.factory.getSpielbrett();
			List<IWissensstreiter> drawableWS = gameModel
					.getDrawableWissensstreiter();
			for (IWissensstreiter wissensstreiter : drawableWS) {
				IField field = gameBoard
						.getFieldOfWissensstreiter(wissensstreiter);

				if (field == null) {
					System.out.println("playerno:"
							+ gameModel.getCurrentPlayer().getPlayerNumber());
					for (int i = 0; i < IAPIFactory.factory.getPlayers()
							.getCountOfWissensstreiterInHomeBase(
									IAPIFactory.factory.getGameModel()
											.getCurrentPlayer()); i++) {
						List<Circle> circles = new ArrayList<Circle>();
						circles.add(getBaseFields(
								gameModel.getCurrentPlayer().getPlayerNumber())
								.get(i));
					...
					highlightNodes(Arrays.asList(new Circle[] { this.fields
							.get(fieldIndex) }));
				}
			}
...
enum MainButtonState {
	PROMPT_FOR_DICE_THROW, DICE_THROWN, PROMPT_FOR_NEXT_TURN, PROMPT_FOR_ACCEPT_CATEGORY
}